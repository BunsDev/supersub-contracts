...
describe('Subscription Plugin Tests', function () {
    ...
    const encodePluginManifest = (manifest: PluginManifestStructOutput) => {
        ...
        const values = [
            ...
            manifest.permittedExternalCalls.map(call => [
                call.externalAddress,
                call.permitAnySelector,
                call.selectors
            ]),
            ...
            manifest.executionHooks.map(hook => [
                hook.executionSelector,
                [
                    hook.preExecHook.functionType,
                    hook.preExecHook.functionId,
                    hook.preExecHook.dependencyIndex
                ],
                [
                    hook.postExecHook.functionType,
                    hook.postExecHook.functionId,
                    hook.postExecHook.dependencyIndex
                ]
            ])
        ]
    ...
    const getCallData = (funcSig: string, types: string[], values: any[]) => {
        const abiCoder = hre.ethers.AbiCoder.defaultAbiCoder();
        const encodeCall = abiCoder.encode(
            types,
            values
        );
    ...
    describe('Product/Plan Creation & Update', async () => {
        it("EOA can create Product", async () => {
            ...
            const txn = await subscriptionPlugin.connect(signer).createProduct(
                productName,
                tokenAddress,
                reciepient.address,
                chainId
            ...
            const product = await subscriptionPlugin.providerProducts(
                signer.address,
        ...
        it("EOA can create plan", async () => {
            ...
            // Create Product
            await subscriptionPlugin.connect(signer).createProduct(
            ...
            const txn = await subscriptionPlugin.connect(signer).createPlan(
                hre.ethers.toBeHex(0, 32),
                chargeInterval,
                price
            );
            ...
            const plan = await subscriptionPlugin.providerPlans(
                signer.address,
                hre.ethers.toBeHex(0, 32)
            );
        ...
        it("EOA can update Product & Plan", async () => {
            ...
            const chargeInterval = 3600;
            const price = BigInt(100) * BigInt(10)**(await token.decimals())
            await subscriptionPlugin.connect(signer).createPlan(
                hre.ethers.toBeHex(0, 32),
                chargeInterval,
                price
            );
            ...
            // Update Product
            const txn1 = await subscriptionPlugin.connect(signer).updateProduct(
                hre.ethers.toBeHex(0, 32),
                await token.getAddress(),
            ...
            const txn2 = await subscriptionPlugin.connect(signer).updatePlan(
                hre.ethers.toBeHex(0, 32),
                newPrice,
                500,
                true
            ...
            await expect(txn2)
                ...
                .withArgs(plan.planId, newPrice, 500, true);
        })
    ...
    describe('User Subscription', async () => {
        it('User can subscribe, unsubscribe, and change subscription plan', async () => {
            ...
            await expect(subscribeTxn)
                ...
                .withArgs(
                    expectedSubscriber, await mscaAccount.getAddress(), 
                    hre.ethers.toBeHex(0, 32), hre.ethers.toBeHex(0, 32), 
                    expectedSubId
                );
